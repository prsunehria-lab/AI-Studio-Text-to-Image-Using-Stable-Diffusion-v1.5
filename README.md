...
A lightweight, fully offline Stable Diffusion v1.5 Web UI optimized for low-VRAM NVIDIA GPUs. Built with Flask, Diffusers, and Tailwind CSS. Features Txt2Img, Img2Img, interactive Inpainting Canvas, ControlNet (OpenPose &amp; Recolor), and an automated MTCNN face-refining pipeline.

# AI Studio Generator (Vision X Engine)

An ultra-optimized, local AI image generation studio powered by NVIDIA CUDA, Python, and Flask. This project provides a streamlined, single-file web interface designed to run advanced diffusion workflows entirely offline, with aggressive VRAM management tailored for mid-range hardware (like the RTX 4050 mobile).
